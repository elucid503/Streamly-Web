...
export class AdminPanel extends Component<AdminPanelProps, AdminPanelState> {
  ...
  load = async () => {
    ...
    try {
      ...
      const [codes, interruption] = await Promise.all([
        ...
        Net.Admin.listAccessCodes(),
        Net.Admin.getServiceInterruption().catch(() => ({ enabled: false, title: "", message: "" } as ServiceInterruption)),
  ...
  saveInterruption = async () => {
    ...
    try {
      ...
      const saved = await Net.Admin.updateServiceInterruption({
  ...
  create = async () => {
    ...
    try {
      ...
      await Net.Admin.createAccessCode(maxUses, this.state.expiresIn || undefined);
  ...
  remove = async (code: string) => {
    ...
    await Net.Admin.deleteAccessCode(code);
