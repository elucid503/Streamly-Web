...
import { FeaturedHero } from "@/Features/Catalog/FeaturedHero";
import { TitleCard } from "@/Features/Catalog/TitleCard";

import { catalogAPI } from "@/Features/Catalog/Api";
import { continueWatching, latestTitleProgress, progressLabel, resumePath } from "@/Features/Library/History";
import type { FavoriteItem, WatchHistoryItem } from "@/Features/Library/Types";
import type { FeedItem, FeedSection, HomeFeed } from "@/Features/Catalog/Types";
...
export class FeedView extends Component<FeedViewProps, FeedViewState> {
    ...
    const entries = await Promise.all(
      ...
      resumeItems.map(async (item) => {
        ...
        try {
          ...
          const details = await catalogAPI.showDetails(item.mediaId);
  ...
  load = async () => {
    ...
    try {
      ...
      const [movieFeed, showFeed] = await Promise.all([
        ...
        catalogAPI.homeFeed("movie"),
        catalogAPI.homeFeed("show"),
  ...
  ensureShowboxId = async (item: FeedItem): Promise<number> => {
    ...
    const resolved = await catalogAPI.resolveTitle({
  ...
  renderSections = (feed: HomeFeed | null, prefix: string, loading: boolean) => {
  ...
  render() {
    ...
    return (
      ...
      <div className="animate-fade-in py-8">
        ...
        {this.renderSections(showFeed, "shows", loading)}

        {this.renderSections(movieFeed, "movies", loading)}
