...
import { PosterImage } from "@/Features/Catalog/PosterImage";
...
import { episodeProgressPercent, resumePath, showEpisodeHistory, showResumeItem, } from "@/Features/Library/History"

import { cn } from "@/Utils/ClassNames";
import type { NavigateFn } from "@/Utils/Navigation";
import type { Episode, Season, TitleDetails } from "@/Features/Catalog/Types";
...
export class DetailPage extends Component<DetailPageProps, DetailPageState> {
  ...
  async componentDidMount() {
    ...
    void import("@/Features/Player/Watch");
    ...
    try {
      ...
      if (kind === "movie") {
        ...
        const [details, history] = await Promise.all([
          ...
          catalogAPI.movieDetails(id),
          historyAPI.get().catch(() => []),
        ...
        const [details, history] = await Promise.all([
          ...
          catalogAPI.showDetails(id),
          historyAPI.get(500, id).catch(() => [])
        ...
        try {
          ...
          seasons = await catalogAPI.showSeasons(id);

        } catch (err) {

          seasonsError = err instanceof Error ? err.message : "seasons temporarily unavailable";
  ...
  loadInitialSeason = async (showId: number, seasons: Season[], selectedSeason: number, gen: number) => {
    ...
    try {
      ...
      const episodes = await catalogAPI.seasonEpisodes(showId, selectedSeason);
  ...
  prefetchSeasons = ( showId: number, seasons: Season[], activeSeason: number, cached: Record<number, Episode[]> ) => {
    ...
    seasons.filter((season) => season.number !== activeSeason && !cached[season.number]).slice(0, 3).forEach((season) => { // prefetches up to 3 seasons, not active one
        ...
        void catalogAPI.seasonEpisodes(showId, season.number).then((episodes) => {
  ...
  selectSeason = async (season: number) => {
    ...
    try {
      ...
      const episodes = await catalogAPI.seasonEpisodes(id, season);
      ...
      <div className="min-h-screen overflow-x-hidden">
        ...
        <div className="relative h-[42vh] min-h-[280px] max-h-[480px] overflow-hidden sm:h-[48vh]">
          ...
          <div className="absolute inset-x-0 bottom-0 flex items-end gap-5 px-4 pb-6 sm:gap-6 sm:px-8 sm:pb-8">
            ...
            <motion.div className="hidden w-36 flex-shrink-0 overflow-hidden rounded-md border border-border shadow-lg sm:block sm:w-44"

              initial={{ opacity: 0, y: 12 }}
              animate={{ opacity: 1, y: 0 }}

            >
              <PosterImage className="aspect-[2/3] w-full"
