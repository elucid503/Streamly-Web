...
import type { AccessCode, ServiceInterruption } from "@/Features/Admin/Types";
...
export class AdminPanel extends Component<AdminPanelProps, AdminPanelState> {
  ...
  load = async () => {
    ...
    try {
      ...
      const [codes, interruption] = await Promise.all([
        ...
        adminAPI.getServiceInterruption().catch(() => ({ enabled: false, title: "", message: "" } as ServiceInterruption)),
  ...
  saveInterruption = async () => {
    ...
    try {
      ...
      const saved = await adminAPI.updateServiceInterruption({
  ...
  create = async () => {
    ...
    try {
      ...
      await adminAPI.createAccessCode(maxUses, this.state.expiresIn || undefined);
  ...
  remove = async (code: string) => {
    ...
    await adminAPI.deleteAccessCode(code);
