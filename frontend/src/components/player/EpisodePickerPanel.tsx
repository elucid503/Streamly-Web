...
export class EpisodePickerPanel extends Component<
  EpisodePickerPanelProps,
  EpisodePickerPanelState
> {
  ...
  componentDidUpdate(prev: EpisodePickerPanelProps) {
    ...
    if (
      this.props.open &&
      (!prev.open ||
        prev.menuSeason !== this.props.menuSeason ||
        prev.episodesLoading !== this.props.episodesLoading ||
        prev.episodes !== this.props.episodes)
    ) {
  ...
  renderEpisodeCard = (ep: Episode) => {
    ...
    return (
      <div
        ...
        className="flex w-[min(78vw,16.5rem)] flex-shrink-0 snap-start sm:w-64"
      >
        <button
          type="button"
          onClick={(e) => {
          ...
          className={cn(
            ...
            active
              ? "border-foreground bg-white/10 ring-1 ring-foreground/30"
  ...
  renderDetailModal = () => {
    ...
    const isCurrent =
      currentSeason === detailEpisode.season && currentEpisode === detailEpisode.episode;
      ...
      >
        <div className="space-y-4">
          ...
          {description ? (
            ...
            <p className="text-sm text-foreground-faint">No description available</p>
          )}
        ...
        <div className="w-full animate-fade-in" onClick={(e) => e.stopPropagation()}>
          <div className="-mx-4 border-t border-white/10 bg-black/75 shadow-[0_-16px_48px_rgba(0,0,0,0.45)] backdrop-blur-xl sm:-mx-6">
            <div className="flex items-center justify-between gap-4 px-4 py-3 sm:px-6">
              ...
              className="rounded-md p-1 text-foreground-muted transition-colors hover:bg-white/8 hover:text-foreground"
            ...
            <div className="relative">
              {canScrollLeft && (
                <>
                  ...
                  <button
                    ...
                    className="absolute top-1/2 left-2 z-10 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-white/25 bg-white/15 text-foreground shadow-[0_4px_24px_rgba(0,0,0,0.45)] backdrop-blur-md transition-colors hover:border-white/40 hover:bg-white/25 sm:left-3"
              ...
              {canScrollRight && (
                <>
                  ...
                  <button
                    ...
                    className="absolute top-1/2 right-2 z-10 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-white/25 bg-white/15 text-foreground shadow-[0_4px_24px_rgba(0,0,0,0.45)] backdrop-blur-md transition-colors hover:border-white/40 hover:bg-white/25 sm:right-3"
              ...
              <div
                ...
                className="flex items-stretch gap-4 overflow-x-auto scroll-smooth pb-5 pl-4 pr-4 pt-1 snap-x snap-mandatory scrollbar-hide scroll-pl-4 sm:gap-5 sm:pb-6 sm:pl-6 sm:pr-6 sm:scroll-pl-6"
              >
                {episodesLoading &&
                  Array.from({ length: 4 }).map((_, index) => (
                    <div
                      key={`ep-picker-skeleton-${index}`}
                      data-episode-card
                      className="flex w-[min(78vw,16.5rem)] flex-shrink-0 snap-start sm:w-64"
                    >
                ...
                {!episodesLoading && episodes.length === 0 && (
                  <div className="flex w-full items-center justify-center py-10">
                    <p className="text-sm text-foreground-muted">No episodes found for this season</p>
                  </div>
                )}
              </div>
            </div>
