...
export class PauseOverlay extends Component<PauseOverlayProps> {
  render() {
    const { visible, poster, still, title, subtitle, episodeTitle, description, onResume, pausedAt: progress, totalDuration } =
      this.props;
      ...
      <>
        {visible && (
          <button
            ...
            className="absolute inset-0 z-[40] flex animate-fade-in cursor-pointer items-center justify-center overflow-hidden bg-black/35 px-4 backdrop-blur-md sm:px-8"
          ...
          >
            ...
            >
              {poster && (
                <div
                  className={cn(
                    ...
                    still
                      ? "aspect-video w-full max-w-xl md:w-[min(42vw,28rem)]"
                      : "aspect-[2/3] w-28 sm:w-36",
                  )}
                >
                  <img
                    src={poster}
              ...
              <div className={cn("min-w-0 space-y-1.5", still && "flex-1")}>
                ...
                {episodeTitle && (
                  <p className="text-base text-foreground/90 sm:text-lg">{episodeTitle}</p>
                )}
                {description && (
                  <p
                    className={cn(
                      ...
                      still
                        ? "max-w-3xl sm:line-clamp-none"
                        : "line-clamp-4 max-w-xl",
                    )}
                  >
                    {description}
                  </p>
            ...
            <p className="pointer-events-none absolute inset-x-0 top-24 z-10 text-center text-xs tracking-wide text-foreground/40 sm:bottom-28 sm:text-sm">
              Paused at {formatDuration(progress * 1000)} / {totalDuration ? formatDuration(totalDuration * 1000) : "Unknown"}
            </p>
