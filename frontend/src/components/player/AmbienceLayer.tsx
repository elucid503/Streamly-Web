...
const luminance = (color: Rgb) => 0.299 * color.r + 0.587 * color.g + 0.114 * color.b;
...
export class AmbienceLayer extends Component<AmbienceLayerProps> {
  ...
  quadrantAverage = (data: Uint8ClampedArray, x0: number, y0: number, x1: number, y1: number): Rgb => {
  ...
  colorsFromPixels = (data: Uint8ClampedArray): { primary: Rgb; secondary: Rgb } => {
    ...
    const ranked = quadrants.map((color) => ({ color, score: this.colorScore(color) })) .sort((a, b) => b.score - a.score);
  ...
  render() {
    ...
    return (
      ...
      <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden bg-black" aria-hidden >
        ...
        <div className="absolute inset-[-26%] scale-[1.16] blur-[92px] saturate-[1.5]"
