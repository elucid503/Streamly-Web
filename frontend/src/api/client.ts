...
export const api = {
  ...
  episodeStream(showId: number, season: number, episode: number, height?: number) {
    const q = height ? `?height=${height}` : "";
    return request<StreamInfo>(`/api/shows/${showId}/seasons/${season}/episodes/${episode}/stream${q}`);
  },
  ...
  nextEpisode(showId: number, season: number, episode: number) {
    return request<NextEpisode | null>(
      `/api/shows/${showId}/seasons/${season}/episodes/${episode}/next`,
    );
  },
