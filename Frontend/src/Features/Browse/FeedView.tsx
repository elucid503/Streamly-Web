...
import { continueWatching, latestTitleProgress, progressLabel, resumePath } from "@/Utils/History";
import type { FavoriteItem, FeedItem, FeedSection, HomeFeed, WatchHistoryItem } from "@/Types";
...
export class FeedView extends Component<FeedViewProps, FeedViewState> {
    ...
    const entries = await Promise.all(
      ...
      resumeItems.map(async (item) => {
        ...
        try {
          ...
          const details = await Net.Catalog.showDetails(item.mediaId);
  ...
  load = async () => {
    ...
    try {
      ...
      const [movieFeed, showFeed] = await Promise.all([
        ...
        Net.Catalog.homeFeed("movie"),
        Net.Catalog.homeFeed("show"),
  ...
  ensureShowboxId = async (item: FeedItem): Promise<number> => {
    ...
    const resolved = await Net.Catalog.resolveTitle({
