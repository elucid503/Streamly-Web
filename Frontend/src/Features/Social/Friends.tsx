...
import type { FriendRequestItem, FriendSummary, LiveChannel, ProfileMedia, PublicProfile, SearchHit, UserProfile, WatchHistoryItem } from "@/Types";
...
class ExpandableFriendCard extends Component<ExpandableFriendCardProps, ExpandableFriendCardState> {
  ...
  loadProfile = async () => {
    ...
    try {
      ...
      const profile = await Net.Social.getPublicProfile(this.props.summary.userId);
  ...
  handleSearch = (q: string) => {
    ...
    this.debounce = setTimeout(async () => {
      ...
      try {
        ...
        const all = await Net.Catalog.search(q);
  ...
  handleAction = async () => {
    ...
    try {
      ...
      if (profile.friendStatus === "none") {
        ...
        await Net.Social.sendFriendRequest(profile.userId);

        this.setState({ profile: { ...profile, friendStatus: "pending_sent" } });

      } else if (profile.friendStatus === "friends") {

        await Net.Social.removeFriend(profile.userId);
...
export class FriendsPage extends ModuleComponent<FriendsPageProps, FriendsPageState> {
  ...
  loadPosters = async () => {
    ...
    try {
      ...
      const [favorites, history, movieTrending, showTrending] = await Promise.all([
        ...
        Net.Favorites.get().catch(() => []),
        Net.History.get(50).catch(() => []),
  ...
  loadFriends = async () => {
    ...
    try {
      ...
      const profiles = await Promise.all(
        ...
        friends.map((f) => Net.Social.getPublicProfile(f.userId).catch(() => null))
  ...
  loadDiscover = async (q: string) => {
    ...
    try {
      ...
      const results = await Net.Social.searchUsers(q);
  ...
  handleUserAction = async (summary: FriendSummary) => {
    ...
    try {
      ...
      if (summary.friendStatus === "none") {
        ...
        if (req) await Net.Social.acceptFriendRequest(req.id);
  ...
  handleSaveProfile = async (form: EditForm) => {
    ...
    try {
      ...
      const updated = await Net.Social.updateProfile({
