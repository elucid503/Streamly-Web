import Net from "@/Net";
import { PosterImage } from "@/Cards/PosterImage";
import { Button } from "@/UI/Button";
import { CachedImage } from "@/UI/CachedImage";
import { HScrollRow } from "@/UI/HScrollRow";

import { episodeProgressPercent, resumePath, showEpisodeHistory, showResumeItem, } from "@/Utils/History"

import { cn } from "@/Utils/ClassNames";
import type { NavigateFn } from "@/Utils/Navigation";
import type { Episode, Season, TitleDetails, WatchHistoryItem } from "@/Types";
...
export class DetailPage extends Component<DetailPageProps, DetailPageState> {
    ...
    try {
      ...
      if (kind === "movie") {
        ...
        const [details, history] = await Promise.all([
          ...
          Net.Catalog.movieDetails(id),
          Net.History.get().catch(() => []),
        ...
        const [details, history] = await Promise.all([
          ...
          Net.Catalog.showDetails(id),
          Net.History.get(500, id).catch(() => [])
        ...
        try {
          ...
          seasons = await Net.Catalog.showSeasons(id);
  ...
  loadInitialSeason = async (showId: number, seasons: Season[], selectedSeason: number, gen: number) => {
    ...
    try {
      ...
      const episodes = await Net.Catalog.seasonEpisodes(showId, selectedSeason);
  ...
  prefetchSeasons = ( showId: number, seasons: Season[], activeSeason: number, cached: Record<number, Episode[]> ) => {
    ...
    seasons.filter((season) => season.number !== activeSeason && !cached[season.number]).slice(0, 3).forEach((season) => { // prefetches up to 3 seasons, not active one
        ...
        void Net.Catalog.seasonEpisodes(showId, season.number).then((episodes) => {
  ...
  selectSeason = async (season: number) => {
    ...
    try {
      ...
      const episodes = await Net.Catalog.seasonEpisodes(id, season);
