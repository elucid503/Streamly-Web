...
func NewAdminHandler(auth *auth.AuthService, db *database.DB) *AdminHandler {
...
func (h *AdminHandler) CreateAccessCode(c *gin.Context) {
	...
	if req.ExpiresIn != "" {
		...
		if err != nil {
			...
			httpx.WriteError(c, http.StatusBadRequest, "invalid expiresIn duration")
			return
...
func (h *AdminHandler) GetServiceInterruption(c *gin.Context) {
	...
	if err != nil {
		...
		httpx.WriteError(c, http.StatusInternalServerError, "failed to load service interruption")
		return
...
func (h *AdminHandler) UpdateServiceInterruption(c *gin.Context) {
	...
	if _, err := h.db.ServiceInterruption().UpdateOne(c.Request.Context(), bson.M{}, update, opts); err != nil {
		...
		httpx.WriteError(c, http.StatusInternalServerError, "failed to save service interruption")
		return
